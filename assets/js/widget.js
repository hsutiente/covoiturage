			...
			InitSelect:function($obj){
				...
				**/
				if(In_array(ApiUtils.lstselect, "rg") === true){
					...
					ApiUtils.sltsec	=	"dp";
				}else if(In_array(ApiUtils.lstselect, "pays") === true){
					ApiUtils.recupLstAll("pays", "all");
					return false;
				}
				/**
		...
		*/
		(function(d){
			...
			if(t3.params && t3.params.needcss){
				jApi('<link/>', {"href":"./css/apiUtils.css", "rel":"stylesheet", "type":"text/css", "media":"screen"}).appendTo('head')/*.attr('href' , "apiUtils.css")*/;
			}
		}(document));
	})(window.jApi);
});
